...
static real_t lookup1_linear(lookup1_t* table, real_t x)
{
  ...
  ASSERT(index >= 0);
  ASSERT(index < (table->num_values-1));
                       ...
                       real_t* y_values, lookup1_interpolation_t interpolation)
{
  ...
  table->dx = (x_max - x_min) / num_values;
  ...
  if (table->interp == LOOKUP1_LINEAR)
  {
    ...
    table->fit_data = polymec_malloc(sizeof(real_t) * (num_values-1));
  ...
  else
  {
    ...
    table->fit_data = polymec_malloc(sizeof(real_t) * 2 * (num_values-1));
    
    // Interior points are just quadratic fits to triples of points.
    real_t dx = table->dx;
    ...
    // On the ends, we just copy the fit coefficients from the interior.
    table->fit_data[0] = table->fit_data[2];
    table->fit_data[1] = table->fit_data[3];
    table->fit_data[2*(num_values-1)] = table->fit_data[2*(num_values-2)];
    table->fit_data[2*(num_values-1)+1] = table->fit_data[2*(num_values-2)+1];
  }
  return table;
}
