...
// provide a single context for lifetimes of collected objects shared between 
...
{
  refcounted_dtor_t* storage_f = memory;
...
void* retain_ref(void* refcounted_resource)
{
  ...
  // Lua registry.
  int* storage_ref = (int*)collected_obj_header(refcounted_resource);
  int ref = storage_ref[0];
  lua_State* L = polymec_lua_State();
  lua_rawgeti(L, LUA_REGISTRYINDEX, ref);
  if (luaL_checkudata(L, -1, REFCOUNTED_C_OBJ) == NULL)
    polymec_error("retain_ref called on non-refcounted object!");
  lua_pop(L, 1);
...
void release_ref(void* refcounted_resource)
{
  ...
  lua_rawgeti(L, LUA_REGISTRYINDEX, ref);
  if (luaL_checkudata(L, -1, REFCOUNTED_C_OBJ) == NULL)
    polymec_error("release_ref called on non-refcounted object!");
  lua_pop(L, 1);
