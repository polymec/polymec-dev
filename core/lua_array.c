...
static int array_new(lua_State* L)
{
  ...
  {
    array = array_from_bytes(L, 1);
    type = LUA_ARRAY_BYTE;
  }
  else if (strcmp(type_str, "int") == 0)
  {
    array = array_from_ints(L, 1);
    type = LUA_ARRAY_INT;
  }
  else if (strcmp(type_str, "int64") == 0) 
  {
    array = array_from_int64s(L, 1);
    type = LUA_ARRAY_INT64;
  }
  else if (strcmp(type_str, "uint64") == 0) 
  {
    array = array_from_uint64s(L, 1);
    type = LUA_ARRAY_UINT64;
  }
  else if (strcmp(type_str, "index") == 0) 
  {
    array = array_from_indices(L, 1);
    type = LUA_ARRAY_INDEX;
  }
  else if (strcmp(type_str, "real") == 0) 
  {
    array = array_from_reals(L, 1);
    type = LUA_ARRAY_REAL;
  }
  else if (strcmp(type_str, "complex") == 0) 
  {
    array = array_from_complexes(L, 1);
    type = LUA_ARRAY_COMPLEX;
  }
  else if (strcmp(type_str, "point") == 0) 
  {
    array = array_from_points(L, 1);
    type = LUA_ARRAY_POINT;
  }
  else if (strcmp(type_str, "vector") == 0) 
  {
    array = array_from_vectors(L, 1);
    type = LUA_ARRAY_VECTOR;
  }
  else if (strcmp(type_str, "tensor2") == 0) 
  {
    array = array_from_tensor2s(L, 1);
    type = LUA_ARRAY_TENSOR2;
  }
  else if (strcmp(type_str, "sym_tensor2") == 0)
  {
    array = array_from_sym_tensor2s(L, 1);
    type = LUA_ARRAY_SYM_TENSOR2;
  }
  else
  {
    return luaL_error(L, "Argument 2 must be one of the following:\n"
