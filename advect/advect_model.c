...
static advect_bc_t* create_bc(double alpha, double beta, st_func_t* F)
{
  ASSERT(F != NULL);
...
typedef struct
{
  ...
  int* boundary_faces;
  advect_bc_t** bc_for_face;
} advect_boundary_cell_t;

static advect_boundary_cell_t* create_boundary_cell()
{
  advect_boundary_cell_t* bcell = malloc(sizeof(advect_boundary_cell_t));
...
static void free_boundary_cell(advect_boundary_cell_t* cell)
{
...
static void advect_run_laplace_1d(options_t* options, int dim)
{
...
{
  advect_run_laplace_1d(options, 1);
}

static void run_laplace_1d_2(options_t* options)
{
  advect_run_laplace_1d(options, 2);
}

static void run_laplace_1d_3(options_t* options)
{
  advect_run_laplace_1d(options, 3);
}
...
static void advect_run_paraboloid(options_t* options, int dim)
{
  ASSERT((dim == 2) || (dim == 3));
...
{
  advect_run_paraboloid(options, 2);
}

static void run_paraboloid_3(options_t* options)
{
  advect_run_paraboloid(options, 3);
}
...
static void initialize_boundary_cells(str_ptr_unordered_map_t* bcs, mesh_t* mesh, int_ptr_unordered_map_t* boundary_cells)
{
  ...
  while (str_ptr_unordered_map_next(bcs, &pos, &tag, (void**)&bc))
  {
    ...
    for (int f = 0; f < num_faces; ++f)
    {
      ...
      advect_boundary_cell_t* boundary_cell;
  ...
  // Allocate storage for the boundary faces and BCs within the cells.
  {
    ...
    while (int_ptr_unordered_map_next(boundary_cells, &pos, &bcell_index, (void**)&bcell))
    {
      ...
      bcell->bc_for_face = malloc(sizeof(advect_bc_t*)*bcell->num_boundary_faces);
    }
  }
  ...
  while (str_ptr_unordered_map_next(bcs, &pos, &tag, (void**)&bc))
  {
    ...
    for (int f = 0; f < num_faces; ++f)
    {
      ...
      advect_boundary_cell_t* boundary_cell = *int_ptr_unordered_map_get(boundary_cells, bcell);
                      ...
                      Vec b)
{
  ...
  VecAssemblyBegin(b);
  int pos = 0, bcell;
  advect_boundary_cell_t* cell_info;
