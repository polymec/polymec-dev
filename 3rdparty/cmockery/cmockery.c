...
static void add_symbol_value(ListNode * const symbol_map_head,
	...
	assert_true(symbol_map_head != NULL);
	assert_true(symbol_names != NULL);
	assert_true(number_of_symbol_names != 0);
...
static int get_symbol_value(
	...
	if (list_find(head, symbol_name, symbol_names_match, &target_node)) {
		...
		ListNode *child_list;
		int return_value = 0;
		assert_true(target_node != NULL);
		assert_true(target_node->value != NULL);
...
static int value_in_set_display_error(
	...
	assert_true(check_integer_set != NULL);
	{
...
static void expect_set(
	...
	assert_true(number_of_values != 0);
...
static int check_in_range(const void *value, void *check_value_data) {
	...
	assert_true(check_value_data != NULL);
...
// Portable implementation of strsignal() follows:
