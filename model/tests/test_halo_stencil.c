                                ...
                                int num_corner_neighbors)
{
  ...
  stencil_t* stencil = cell_halo_stencil_new(mesh);
  for (int i = 0; i < nx; ++i)
  {
    for (int j = 0; j < ny; ++j)
    {
...
{
  test_NXxNYxNZ_halo_stencil(state, MPI_COMM_SELF, 10, 1, 1, 0, 0, 2, 1);
}

void test_serial_10x10x1_cell_halo_stencil(void** state)
{
  test_NXxNYxNZ_halo_stencil(state, MPI_COMM_SELF, 10, 10, 1, 8, 8, 5, 3);
}
...
{
  test_NXxNYxNZ_halo_stencil(state, MPI_COMM_WORLD, 10, 1, 1, 0, 0, 2, 1);
}

void test_parallel_10x10x1_cell_halo_stencil(void** state)
{
  test_NXxNYxNZ_halo_stencil(state, MPI_COMM_WORLD, 10, 10, 1, 8, 8, 5, 3);
}

void test_parallel_10x10x10_cell_halo_stencil(void** state)
{
  test_NXxNYxNZ_halo_stencil(state, MPI_COMM_WORLD, 10, 10, 10, 26, 17, 11, 7);
}
...
int main(int argc, char* argv[]) 
{
  ...
  {
    cmocka_unit_test(test_serial_1x1x1_cell_halo_stencil),
    cmocka_unit_test(test_serial_10x1x1_cell_halo_stencil),
    cmocka_unit_test(test_serial_10x10x1_cell_halo_stencil),
    cmocka_unit_test(test_serial_10x10x10_cell_halo_stencil)
#if POLYMEC_HAVE_MPI
   ,cmocka_unit_test(test_parallel_10x1x1_cell_halo_stencil),
    cmocka_unit_test(test_parallel_10x10x1_cell_halo_stencil),
    cmocka_unit_test(test_parallel_10x10x10_cell_halo_stencil)
#endif
  };
