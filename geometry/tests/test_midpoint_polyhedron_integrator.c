...
#include "geometry/polyhedron_integrator.h"
...
static void test_cube_volume_integrals(void** state)
{
  ...
  assert_true(reals_nearly_equal(I1_1, 0.5, 2.0*REAL_EPSILON));
  real_t I1_2 = cube_volume_integral(I, 0.5, f1);
  assert_true(reals_nearly_equal(I1_2, 0.03125, 2.0*REAL_EPSILON));
  real_t I2_1 = cube_volume_integral(I, 1.0, f2);
  assert_true(reals_nearly_equal(I2_1, 0.75, 2.0*REAL_EPSILON));
  real_t I2_2 = cube_volume_integral(I, 0.5, f2);
  assert_true(reals_nearly_equal(I2_2, 0.0390625, 2.0*REAL_EPSILON));
  polyhedron_integrator_free(I);
}
...
static void test_cube_surface_integrals(void** state)
{
  ...
  assert_true(reals_nearly_equal(I1_1, 1.0, 2.0*REAL_EPSILON));
  real_t I1_2 = cube_surface_integral(I, 0.5, g1);
  assert_true(reals_nearly_equal(I1_2, 0.125, 2.0*REAL_EPSILON));
  real_t I2_1 = cube_surface_integral(I, 1.0, g2);
  assert_true(reals_nearly_equal(I2_1, 2.0, 2.0*REAL_EPSILON));
  real_t I2_2 = cube_surface_integral(I, 0.5, g2);
  assert_true(reals_nearly_equal(I2_2, 0.1875, 2.0*REAL_EPSILON));
  polyhedron_integrator_free(I);
}
