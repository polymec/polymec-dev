...
mesh_t* welded_block_mesh(mesh_t** blocks, int num_blocks)
{
  ...
  for (int i = 0; i < num_blocks; ++i)
  {
    ...
    index_t nxf1 = cubic_lattice_num_x_faces(lattice1);
    index_t nyf1 = cubic_lattice_num_y_faces(lattice1);
    index_t nzf1 = cubic_lattice_num_z_faces(lattice1);
    index_t nfaces1[6] = {nyf1 * nzf1, nyf1 * nzf1, 
                           nxf1 * nzf1, nxf1 * nzf1, 
                           nxf1 * nzf1, nyf1 * nyf1};
    index_t nnodes1[6] = {(nyf1+1) * (nzf1+1), (nyf1+1) * (nzf1+1),
    ...
    for (int j = i+1; j < num_blocks; ++j)
    {
      ...
      ASSERT(lattice2 != NULL);
      index_t nxf2 = cubic_lattice_num_x_faces(lattice2);
      index_t nyf2 = cubic_lattice_num_y_faces(lattice2);
      index_t nzf2 = cubic_lattice_num_z_faces(lattice2);
      index_t nfaces2[6] = {nyf2 * nzf2, nyf2 * nzf2, 
                             nxf2 * nzf2, nxf2 * nzf2, 
                             nxf2 * nzf2, nyf2 * nyf2};
