...
///                 J. This operator is computed whenever the solver deems it necessary to 
...
///                 L. This operator is computed whenever the solver deems it necessary to 
///                 reflect the the state of the system (according to inexact-Newton-like methods).
///                 Arguments are:
///                 - context: A pointer to the context object that stores the state of the solver.
///                 - t: The current time.
...
typedef enum 
{
  /// Generalized minimum residual Krylov solver
  NEWTON_GMRES,    
  /// Flexible GMRES Krylov solver
  NEWTON_FGMRES,
  /// Stabilized Biconjugate Gradient Krylov solver
  NEWTON_BICGSTAB, 
  /// Transpose-Free QMR Krylov solver
  NEWTON_TFQMR
} jfnk_newton_t;
...
/// solver assumes control of all these objects.
...
/// the creation of the matrix. The solver assumes control of all these objects.
...
/// \struct newton_solver_diagnostics
...
/// \memberof newton_solver_diagnostics
