...
extern ode_solver_t* bj_jfnk_bdf_diurnal_solver_new(newton_pc_side_t side);
extern real_t* diurnal_initial_conditions(ode_solver_t* integ);
extern int test_diurnal_step(void** state, ode_solver_t* integ, int max_steps);

static void test_bj_jfnk_bdf_diurnal_ctor(void** state)
{
  ode_solver_t* integ = bj_jfnk_bdf_diurnal_solver_new(NEWTON_PC_LEFT);
  ode_solver_free(integ);
  integ = bj_jfnk_bdf_diurnal_solver_new(NEWTON_PC_RIGHT);
  ode_solver_free(integ);
  integ = bj_jfnk_bdf_diurnal_solver_new(NEWTON_PC_BOTH);
  ode_solver_free(integ);
}
...
{
  ode_solver_t* integ = bj_jfnk_bdf_diurnal_solver_new(NEWTON_PC_RIGHT);
#if POLYMEC_HAVE_DOUBLE_PRECISION
  int max_steps = 500;
#else
  int max_steps = 450;
