...
void heat2d_set_initial_conditions(dae_solver_t* integ, real_t** U, real_t** U_dot);
void heat2d_set_initial_conditions(dae_solver_t* integ, real_t** U, real_t** U_dot)
{
  heat2d_t* data;
  const char* integ_name = dae_solver_name(integ);
  if (string_contains(integ_name, "INK"))
    data = ink_dae_solver_context(integ);
  else
    data = dae_solver_context(integ);
...
// Constructor for Jacobian-Free Newton-Krylov heat2d solver with 
// the given preconditioner.
static dae_solver_t* jfnk_heat2d_solver_new(heat2d_t* data, newton_pc_t* precond)
{
  // Set up a time solver using GMRES with a Krylov space of maximum 
  // dimension 5.
  dae_solver_t* integ = jfnk_dae_solver_new(5, MPI_COMM_SELF, 
...
// Constructor for block-Jacobi-preconditioned heat2d solver.
dae_solver_t* bj_pc_jfnk_heat2d_solver_new(void);
dae_solver_t* bj_pc_jfnk_heat2d_solver_new()
{
  ...
  return jfnk_heat2d_solver_new(data, precond);
}

// Constructor for Inexact Newton-Krylov heat2d solver.
dae_solver_t* ink_heat2d_solver_new(krylov_factory_t* factory);
dae_solver_t* ink_heat2d_solver_new(krylov_factory_t* factory)
{
  heat2d_t* data = heat2d_new();
  ...
  return ink_dae_solver_new(5, MPI_COMM_SELF, 
