...
extern dae_solver_t* bj_pc_jfnk_heat2d_solver_new(void);
extern dae_solver_t* ink_heat2d_solver_new(krylov_factory_t* factory);
extern void heat2d_set_initial_conditions(dae_solver_t* integ, real_t** u, real_t** u_dot);
...
{
  dae_solver_t* integ = bj_pc_jfnk_heat2d_solver_new();
  dae_solver_free(integ);
}

static void test_heat2d_step(void** state, dae_solver_t* integ)
{
  // Set up the problem.
  dae_solver_set_tolerances(integ, 1e-5, 1e-3);
  real_t *u, *udot;
  heat2d_set_initial_conditions(integ, &u, &udot);

  // Integrate it.
  real_t t = 0.0, tf = 10.24;
  dae_solver_set_stop_time(integ, tf);
  while (t < tf)
  {
    bool integrated = dae_solver_step(integ, tf, &t, u, udot);
...
printf("%g ", u[i]);
printf("]\n");
  dae_solver_diagnostics_t diags;
  dae_solver_get_diagnostics(integ, &diags);
  dae_solver_diagnostics_fprintf(&diags, stdout);
...
{
  dae_solver_t* integ = ink_heat2d_solver_new(factory);
  dae_solver_free(integ);
}
