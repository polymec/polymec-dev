...
real_t* diurnal_initial_conditions(ode_solver_t* integ);
real_t* diurnal_initial_conditions(ode_solver_t* integ)
{
  ...
  if (string_contains(integ_name, "INK Backwards-Difference"))
    ...
    data = ark_ode_solver_context(integ);
...
// Constructor for a BDF diurnal solver with the given preconditioner.
static ode_solver_t* jfnk_bdf_diurnal_solver_new(diurnal_t* data, newton_pc_t* precond)
{
  // Set up a time solver using GMRES with a maximum order of 5 and 
  // a Krylov space of maximum dimension 30.
  ode_solver_t* integ = jfnk_bdf_ode_solver_new(5, MPI_COMM_SELF, NEQ, 0,
...
// Constructor for block-Jacobi-preconditioned BDF diurnal solver.
ode_solver_t* bj_jfnk_bdf_diurnal_solver_new(newton_pc_side_t side);
ode_solver_t* bj_jfnk_bdf_diurnal_solver_new(newton_pc_side_t side)
{
  ...
  ode_solver_t* integ = jfnk_bdf_diurnal_solver_new(data, precond);
  return integ;
}

// Constructor for an Inexact Newton-Krylov BDF diurnal solver.
ode_solver_t* ink_bdf_diurnal_solver_new(krylov_factory_t* factory);
ode_solver_t* ink_bdf_diurnal_solver_new(krylov_factory_t* factory)
{
  diurnal_t* data = diurnal_new();
  // Set up a time solver using GMRES with a maximum order of 5 and 
  ...
  ode_solver_t* integ = ink_bdf_ode_solver_new(5, MPI_COMM_SELF, factory,
                                                       J_sparsity, data, diurnal_rhs, 
                                                       diurnal_J, diurnal_dtor);
  ink_bdf_ode_solver_use_gmres(integ, 30);
  return integ;
}

// Constructor for an ARK diurnal solver with the given preconditioner.
static ode_solver_t* jfnk_ark_diurnal_solver_new(diurnal_t* data, newton_pc_t* precond)
{
  // Set up a time solver using GMRES with a maximum order of 3 and 
  // a Krylov space of maximum dimension 30.
  ode_solver_t* integ;
  if (precond != NULL)
  {
    integ = jfnk_ark_ode_solver_new(3, MPI_COMM_SELF, NEQ, 0,
  ...
  {
    integ = functional_ark_ode_solver_new(3, MPI_COMM_SELF, NEQ, 0,
...
// Constructor for functional ARK diurnal solver.
ode_solver_t* functional_ark_diurnal_solver_new(void);
ode_solver_t* functional_ark_diurnal_solver_new()
{
  diurnal_t* data = diurnal_new();
  ode_solver_t* integ = jfnk_ark_diurnal_solver_new(data, NULL);
  return integ;
}

// Constructor for block-Jacobi-preconditioned ARK diurnal solver.
ode_solver_t* bj_jfnk_ark_diurnal_solver_new(newton_pc_side_t side);
ode_solver_t* bj_jfnk_ark_diurnal_solver_new(newton_pc_side_t side)
{
  diurnal_t* data = diurnal_new();
  newton_pc_t* precond = cpr_bj_newton_pc_new(MPI_COMM_WORLD, data, diurnal_rhs, NULL, side, data->sparsity, NEQ/NUM_SPECIES, 0, NUM_SPECIES);
  ode_solver_t* integ = jfnk_ark_diurnal_solver_new(data, precond);
  return integ;
}
...
ode_solver_t* ink_ark_diurnal_solver_new(krylov_factory_t* factory)
{
  ...
  // a Krylov space of maximum dimension 30.
  matrix_sparsity_t* J_sparsity = matrix_sparsity_from_graph(data->sparsity, NULL);
  ode_solver_t* integ = ink_ark_ode_solver_new(5, MPI_COMM_SELF, factory,
  ...
  if ((solver == NULL) || (string_casecmp(solver, "gmres") == 0))
    ...
    ink_ark_ode_solver_use_bicgstab(integ);
  else
    ink_ark_ode_solver_use_gmres(integ, 30);
  return integ;
}
...
{
  // Set up the problem.
  char* integ_name = ode_solver_name(integ);
#if POLYMEC_HAVE_DOUBLE_PRECISION
  if (string_contains(integ_name, "Backwards"))
    bdf_ode_solver_set_tolerances(integ, 1e-5, 1e-3);
  else if (string_contains(integ_name, "Additive"))
    ark_ode_solver_set_tolerances(integ, 1e-5, 1e-3);
#else
  if (string_contains(integ_name, "Backwards"))
    bdf_ode_solver_set_tolerances(integ, 1e-4, 1e-2);
  else if (string_contains(integ_name, "Additive"))
    ark_ode_solver_set_tolerances(integ, 1e-4, 1e-2);
#endif
  ...
  {
    real_t t_old = t;
    bool integrated = ode_solver_step(integ, 7200.0, &t, U);
  ...
  if (string_contains(integ_name, "Backwards"))
  {
    bdf_ode_solver_diagnostics_t diags;
    bdf_ode_solver_get_diagnostics(integ, &diags);
    bdf_ode_solver_diagnostics_fprintf(&diags, stdout);
  }
  else
  {
    ark_ode_solver_diagnostics_t diags;
    ark_ode_solver_get_diagnostics(integ, &diags);
    ark_ode_solver_diagnostics_fprintf(&diags, stdout);
  }
  ...
  // Did the solver finish before taking the max number of steps?
