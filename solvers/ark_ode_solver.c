...
static bool ark_step(void* context, real_t max_dt, real_t* t, real_t* U)
{
  ...
  if (t2 > integ->t)
  {
    ...
    if ((t2 - *t) < (integ->t - *t))
      log_detail("ark_ode_solver: took internal step dt = %g", integ->t - *t);
  }
                                            ...
                                            int max_anderson_accel_dim)
{
  ...
  ark_ode_solver_set_tolerances(I, 1e-4, 1.0);
...
void ark_ode_solver_set_error_weights(ode_solver_t* solver, real_t* weights)
{
  ...
  ark_ode_solver_set_error_weight_function(solver, use_constant_weights);
}
...
void ark_ode_solver_eval_fi(ode_solver_t* solver, real_t t, real_t* U, real_t* fi)
{
  ark_ode_t* integ = ode_solver_context(solver);
  if (integ->fi != NULL)
  {
                                        ...
                                        FILE* stream)
{
  ...
  fprintf(stream, "ARK ODE solver diagnostics:\n");
                     ...
                     real_t* work1, real_t* work2, real_t* work3)
{
  ...
  log_debug("ink_ark_ode_solver: Calculating M = I - %g * J.", gamma);
  if ((step == 0) ||
      ...
      (conv_status == ARK_CONV_OTHER_FAILURE))
  {
    ...
    log_debug("ink_ark_ode_solver: Updating J (reason: %s).", reason);
                     ...
                     int* num_iters) 
{
  ...
  {
    log_debug("ink_ark_ode_solver: ||B||_2 < tolerance (%g < %g), so X -> 0.", B_norm, res_norm_tol); 
  ...
  if (solved)
  {
    log_debug("ink_ark_ode_solver: Solved A*X = B (||R||_2 == %g after %d iters).", res_norm, num_iters);
  ...
  {
    log_debug("ink_ark_ode_solver: Solution to A*X = B did not converge.");
    STOP_FUNCTION_TIMER();
    return 1;
  }
}
                                     ...
                                     void (*dtor)(void* context))
{
  ...
  ode_solver_t* I = ark_ode_solver_new(name, order, comm, 
                                       num_local_values, 0,
                                       ink, 
