...
// PetscFactory - A string containing the name of the factory constructor.
// PetscInt - The integer type used for indexing.
// PetscScalar - The type used to store floating point values, real or complex.
...
static void petsc_matrix_insert_values(void* context, size_t num_rows,
                                       ...
                                       real_t* values, InsertMode insert_mode)
{
  ...
  {
    PetscInt num_cols = (PetscInt)num_columns[r];
    PetscInt row = (PetscInt)rows[r];
    PetscInt cols[num_cols];
    PetscReal vals[num_cols];
    for (int c = 0; c < num_cols; ++c, ++col_offset)
    {
      cols[c] = (PetscInt)columns[col_offset];
...
static void petsc_matrix_set_values(void* context, size_t num_rows,
                                    size_t* num_columns, index_t* rows, index_t* columns,
                                    real_t* values)
{
...
static void petsc_matrix_add_values(void* context, size_t num_rows,
                                    size_t* num_columns, index_t* rows, index_t* columns,
                                    real_t* values)
{
...
static void petsc_matrix_get_values(void* context, size_t num_rows,
                                    size_t* num_columns, index_t* rows, index_t* columns,
                                    real_t* values)
{
  petsc_matrix_t* A = context;
                                             ...
                                             matrix_sparsity_t* sparsity)
{
  ...
  size_t N_local = matrix_sparsity_num_local_rows(sparsity);
  size_t N_global = matrix_sparsity_num_global_rows(sparsity);
  ...
  A->factory->methods.MatSetSizes(A->A, (PetscInt)N_local, (PetscInt)N_local, (PetscInt)N_global, (PetscInt)N_global);

  index_t* row_dist = matrix_sparsity_row_distribution(sparsity);
  PetscInt start = (PetscInt)row_dist[rank], end = (PetscInt)row_dist[rank+1];
...
static size_t petsc_matrix_block_size(void* context, index_t row)
{
...
static void petsc_matrix_manipulate_fixed_blocks(void* context, size_t num_blocks,
...
static void petsc_matrix_set_fixed_blocks(void* context, size_t num_blocks,
                                          index_t* block_rows, index_t* block_columns, 
                                          real_t* block_values)
{
...
static void petsc_matrix_add_fixed_blocks(void* context, size_t num_blocks,
                                          index_t* block_rows, index_t* block_columns, 
                                          real_t* block_values)
{
  petsc_matrix_manipulate_fixed_blocks(context, num_blocks, 
...
static void petsc_matrix_get_fixed_blocks(void* context, size_t num_blocks,
                                          index_t* block_rows, index_t* block_columns, 
                                          real_t* block_values)
{
  petsc_matrix_manipulate_fixed_blocks(context, num_blocks, 
                                                   ...
                                                   size_t block_size)
{
  ...
  size_t N_local = matrix_sparsity_num_local_rows(block_sp);
  size_t N_global = matrix_sparsity_num_global_rows(block_sp);
  ...
  A->factory->methods.MatSetSizes(A->A, (PetscInt)N_local, (PetscInt)N_local, (PetscInt)N_global, (PetscInt)N_global);
  A->factory->methods.MatSetBlockSize(A->A, (PetscInt)block_size);
...
static void petsc_matrix_manipulate_var_blocks(void* context, size_t num_blocks,
                                               index_t* block_rows, index_t* block_columns, 
                                               real_t* block_values,
                                               void (*manipulate)(void*, size_t, size_t*, index_t*, index_t*, real_t*),
                                               bool copy_out)
{
  ...
  for (size_t i = 0; i < num_blocks; ++i)
  {
    ...
    index_t block_row = block_rows[i];
    index_t block_column = block_columns[i];
    size_t bs = petsc_matrix_block_size(mat, block_row);
    size_t num_rows = bs, num_columns[bs];
    index_t rows[bs], columns[bs*bs];
    {
      int l = 0;
...
static void petsc_matrix_set_var_blocks(void* context, size_t num_blocks,
                                        index_t* block_rows, index_t* block_columns, 
                                        real_t* block_values)
{
...
static void petsc_matrix_add_var_blocks(void* context, size_t num_blocks,
                                        index_t* block_rows, index_t* block_columns, 
                                        real_t* block_values)
{
  petsc_matrix_manipulate_var_blocks(context, num_blocks, 
...
static void petsc_matrix_get_var_blocks(void* context, size_t num_blocks,
                                        index_t* block_rows, index_t* block_columns, 
                                        real_t* block_values)
{
  petsc_matrix_manipulate_var_blocks(context, num_blocks, 
                                                       ...
                                                       size_t* block_sizes)
{
  ...
  A->block_sizes = polymec_malloc(sizeof(size_t) * N_local);
  memcpy(A->block_sizes, block_sizes, sizeof(size_t) * N_local);
...
static void petsc_vector_set_values(void* context, size_t num_values,
                                    index_t* indices, real_t* values)
{
  petsc_vector_t* v = context;
  PetscInt ind[num_values];
  for (size_t i = 0; i < num_values; ++i)
    ind[i] = (PetscInt)indices[i];
  v->factory->methods.VecSetValues(v->v, (PetscInt)num_values, ind, values, INSERT_VALUES);
}

static void petsc_vector_add_values(void* context, size_t num_values,
                                    index_t* indices, real_t* values)
{
  petsc_vector_t* v = context;
  PetscInt ind[num_values];
  for (size_t i = 0; i < num_values; ++i)
    ind[i] = (PetscInt)indices[i];
  v->factory->methods.VecSetValues(v->v, (PetscInt)num_values, ind, values, ADD_VALUES);
}

static void petsc_vector_get_values(void* context, size_t num_values,
                                    index_t* indices, real_t* values)
{
  petsc_vector_t* v = context;
  PetscInt ind[num_values];
  for (size_t i = 0; i < num_values; ++i)
    ind[i] = (PetscInt)indices[i];
  v->factory->methods.VecGetValues(v->v, (PetscInt)num_values, ind, values);
}
                                             ...
                                             index_t* row_dist)
{
  ...
  size_t N_local = (size_t)(row_dist[rank+1] - row_dist[rank]);
  size_t N_global = (size_t)row_dist[nprocs];

  if (nprocs == 1)
    v->factory->methods.VecCreateSeq(comm, (PetscInt)N_global, &v->v);
  else
    v->factory->methods.VecCreateMPI(comm, (PetscInt)N_local, (PetscInt)N_global, &v->v);
...
#define STR(s) SSTR(s)
#define SSTR(s) #s
krylov_factory_t* PetscFactory(const char* petsc_library);
krylov_factory_t* PetscFactory(const char* petsc_library)
{
  ASSERT(file_exists(petsc_library)); // checked by caller

  petsc_factory_t* factory = polymec_malloc(sizeof(petsc_factory_t));

  // Try to open libPETSc and mine it for symbols.
  log_debug(STR(PetscFactory) ": Opening PETSc library at %s.", petsc_library);
  void* petsc = dlopen(petsc_library, RTLD_NOW);
  if (petsc == NULL)
  {
    char* msg = dlerror();
    polymec_error(STR(PetscFactory) ": %s.", msg);
  }
  ...
  if (err != 0)
  {
    log_urgent(STR(PetscFactory) ": Error getting PETSc version string.");
    goto failure;
  }

  log_debug(STR(PetscFactory) ": Got PETSc version: %s", version);
...
#undef FETCH_PETSC_SYMBOL
  ...
  log_debug(STR(PetscFactory) ": Got PETSc symbols.");
  ...
  {
    log_debug(STR(PetscFactory) ": Initializing PETSc.");
