...
dae_integrator_t* block_jacobi_precond_heat2d_integrator_new()
{
  ...
  heat2d_t* data = dae_integrator_context(integ);
  preconditioner_t* precond = block_jacobi_preconditioner_from_dae_function("Heat 2D", data, heat2d_res, NULL, data->sparsity, NEQ, 0, 1);
...
dae_integrator_t* lu_precond_heat2d_integrator_new()
{
  dae_integrator_t* integ = heat2d_integrator_new();
  heat2d_t* data = dae_integrator_context(integ);
  preconditioner_t* precond = lu_preconditioner_from_dae_function("Heat 2D", data, heat2d_res, NULL, data->sparsity, NEQ, 0, 1);
  dae_integrator_set_preconditioner(integ, precond);
  return integ;
}
